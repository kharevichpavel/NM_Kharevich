...
<fmt:message bundle="${loc}" key="local.newslist.pagecount"	var="page_count" />
				...
				<div class="news-link-to-wrapper">
					<div class="link-position">
						<c:if test="${sessionScope.role eq 'admin'}">
							<a style="text-decoration: none;" href="controller?command=go_to_edit_news&id=${news.idNews}">
								<c:out value="${newslist_edit}" />&nbsp;</a>
						</c:if>
						<a style="text-decoration: none;" href="controller?command=go_to_view_news&id=${news.idNews}">
							<c:out value="${newslist_view}" />&nbsp;</a>
...
<form action="controller" method="get">
	<label><c:out value="${page_count}" />
	<select	size="1" name="paginationSizeFromUser" onfocus="this.value=''">
	...
	<p align="center">
		<c:forEach var="i" begin="1" end="${requestScope.paginationSizeForPage}">
			<label>
				<a href="controller?command=go_to_news_list&pageId=${i}&paginationSizeFromUser=${requestScope.paginationSizeFromUser}">
				<c:out value="${page}" /> <c:out value="${i}" /></a>&nbsp;&nbsp;
			</label>
		</c:forEach>
	</p>
